...
function validate_form() { 
    ...
    if (!strlen($input['name'])) { $errors [] = 'Please enter your name.'; } 
    // size is required 
    ...
    if (count($input['main_dish']) != 2) { 
        ...
        // both valid 
        if (!(
            array_key_exists($input['main_dish'][0], $GLOBALS['main_dishes']) &&
...
function process_form($input) { 
    ...
    $message =<<<_ORDER_
Thank you for your order, {$input['name']}. You requested the {$input['size']} size of $sweet, $main_dish_1, and
$main_dish_2. 
...
_ORDER_;
    ...
    mail('TogusaRusso@gmail.com', 'New Order', $message); 
